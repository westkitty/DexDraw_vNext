...
## Entry 43 — Compact floating board HUD and message flood relief

**Date:** 2026-05-08

**Summary:** Applied a manual board usability patch after browser testing showed two serious issues: the board UI consumed too much vertical screen space, and freehand sketching could trigger `Too many messages` immediately.

**Files changed:**
- `apps/client-web/src/styles.css`
- `apps/server-api/src/app.ts` if a detectable message threshold was patched
- `DexDraw_vNext_Bible.md`

**Implementation:**
- Added compact floating HUD CSS overrides for board header, toolbar, presence, metrics, and error banner.
- Kept the canvas as the dominant page surface.
- Raised obvious server-side message/rate thresholds when detectable to avoid punishing normal first-sketch pen use.

**Decision:** Board controls should float above the canvas like an app HUD, not occupy large permanent document rows. Further refinement should eventually group/collapse tools intentionally rather than depending only on CSS density.

**Validation plan:** Run typecheck and lint first. Manually test pen sketching and board layout in the browser. Run full e2e only after the restored gateway flow and HUD layout have their tests updated.


## Entry 45 — Remove FAQ from main welcome title card

**Date:** 2026-05-08

**Summary:** Removed the FAQ trigger from the main welcome/title card because it cluttered the first post-gateway menu and did not provide useful section-specific support there.

**Files changed:**
- `apps/client-web/src/components/HomePage.tsx`
- `DexDraw_vNext_Bible.md`

**Decision:** FAQ support should remain on actionable sections such as Create, Join, board tools, collaboration, and status. The main welcome/title card should stay visually clean.

**Validation plan:** Run typecheck and lint. Manual browser refresh required.



## Entry 46 — Post-welcome page depth and motion polish

**Date:** 2026-05-08

**Summary:** Improved the post-welcome page visual treatment after browser review showed it still looked like a flat web form rather than an app-like entry surface.
...
**Implementation:**
- Removed the first/top-level FAQ trigger from the main welcome/title area.
- Added layered dark app-surface background treatment.
- Added card depth, glass treatment, hover lift, subtle glow, input focus motion, and surface-rise animation.
- Kept section-level FAQ support on actionable areas.
- Added reduced-motion fallback.

**Decision:** The post-welcome page should feel like an app surface with depth and responsive motion, not a static web page form.

**Validation plan:** Run typecheck and lint. Manual browser review required. Full e2e remains deferred until gateway/menu flow tests are updated.



## Entry 47 — Post-welcome menu button ink trail

**Date:** 2026-05-08

**Summary:** Added a pointer-following ink trail effect to post-welcome menu buttons so the Create/Join menu feels more tactile and app-like.
...
**Implementation:**
- Added a HomePage-scoped pointermove effect that spawns temporary ink droplets when hovering over menu buttons.
- Ink droplets fade and drift briefly before being removed.
- Added CSS animation for the droplet bloom/trail effect.
- Added reduced-motion handling so the effect is disabled when motion reduction is requested.

**Decision:** The post-welcome menu should feel interactive and alive, not like a static web form. Button hover should leave a brief visual trace without permanently altering layout or controls.

**Validation plan:** Run typecheck and lint. Manual browser review required.



## Entry 48 — Stronger app menu motion and gateway FAQ removal

**Date:** 2026-05-08

**Summary:** Strengthened the post-welcome menu styling and button ink behavior after manual browser review showed the previous pass was too visually subtle. Removed FAQ from the actual animated gateway/welcome screen.
...
**Implementation:**
- Removed Gateway FAQ clutter from the animated welcome screen.
- Added stronger app-surface shadows, glow, hover lift, and depth to post-welcome cards.
- Added a more visible liquid ink hover wash on post-welcome buttons.
- Increased pointer ink droplet size/frequency for visible cursor trail behavior.
- Preserved reduced-motion fallback.

**Decision:** The first app screen after the animation must feel tactile and alive, not like a flat web form. The gateway animation should remain clean and not carry FAQ clutter.

**Validation plan:** Run typecheck and lint. Manual browser review required before commit.


## Entry 49 — Runtime app-like HUD and ink interaction layer

**Date:** 2026-05-08

**Summary:** Implemented a stronger app-like interaction layer after manual review showed previous styling passes were not visibly effective. The goal is to move DexDraw toward an Infinite Painter-style principle of more canvas and less clutter, with dockable/movable controls and tactile menu buttons.

**Files changed:**
- `apps/client-web/src/interactionEnhancer.ts`
- `apps/client-web/src/main.tsx`
- `apps/client-web/src/styles.css`
- `DexDraw_vNext_Bible.md`

**Implementation:**
- Added `interactionEnhancer.ts` as a guarded browser-side runtime enhancer.
- Added visible ink-trail hover effects to post-welcome menu buttons.
- Added home/menu decoration for stronger card hover depth.
- Added board-route detection.
- Added floating board HUD panel detection for title/share code, tools, export controls, and status/metrics.
- Added draggable handles, collapse controls, resize handles, and localStorage persistence for HUD panels.
- Added CSS for glassy, compact, app-like floating panels.
- Preserved reduced-motion behavior.

**Decision:** DexDraw board controls should behave like movable app HUD boxes over the canvas, not static web-page rows. This pass prioritizes demo-visible interaction quality and canvas space.

**Validation plan:** Run format, typecheck, unit tests, build, lint, and Playwright end-to-end tests. If validation passes, stage, commit, and push.



## Entry 50 — E2E gateway automation and HUD pointer layering fix

**Date:** 2026-05-08

**Summary:** Fixed the validation failure from the app-like HUD pass. Playwright was timing out because restored gateway behavior blocked ordinary tests from reaching the home form, and some board toolbar clicks were intercepted by the canvas.
...
**Implementation:**
- Normal browser visits still show the gateway animation.
- Playwright automation skips the gateway by default unless `?gateway=1` is present.
- Gateway-specific tests now force gateway mode with `/?gateway=1`.
- Updated the stale localStorage gateway test to reflect the intended replayable gateway behavior.
- Added stronger board HUD pointer/z-index rules so floating controls remain clickable above the canvas.

**Decision:** The restored animated gateway is correct for real users, but broad E2E tests should not have to manually enter through it unless they are specifically testing gateway behavior.

**Validation plan:** Run format, typecheck, unit tests, build, lint, targeted E2E, then full E2E. Commit and push only if validation passes.
